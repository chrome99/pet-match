...
exports.search = asyncHandler(async (req, res) => {
  ...
  if (adoptionStatus) {
    searchQuery.adoptionStatus = adoptionStatus;
  }
  if (type) {
    searchQuery.type = type;
  }
  if (height) {
    searchQuery.height = height;
  }
  if (weight) {
    searchQuery.weight = weight;
  }
  if (name) {
...
exports.changeAdoptionStatus = asyncHandler(async (req, res) => {
  ...
  if (value === true) {
    ...
    if (!UserOwnsPet && pet.adoptionStatus !== "Available") {
      return res
        .status(400)
        .send("Pet Already " + pet.adoptionStatus + " By Someone Else");
    }
    ...
    if (UserOwnsPet && pet.adoptionStatus !== "Fostered") {
      return res
        .status(400)
        .send("Pet Already " + pet.adoptionStatus + " By User");
    }
...
exports.update = asyncHandler(async (req, res) => {
  ...
  if (adoptionStatus) {
    petToUpdate.adoptionStatus = adoptionStatus;
  }
  if (height) {
  ...
  if (hypoallergnic) {
    petToUpdate.hypoallergnic = hypoallergnic;
  }
  if (dietery) {
    petToUpdate.dietery = dietery;
  }
  if (breed) {
